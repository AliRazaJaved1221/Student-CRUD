...
export default function Users() {
  ...
  const handleSubmit = (e) => {
    ...
    if (isEditing) {
      const updatedList = list.map((user) =>
        currentUser.id === user.id ? { ...user, ...data } : user,
      );
  ...
  const handleUpdate = (student) => {
    ...
    setCurrentUser(student);
    setIsModalOpen(true);
    setData({name:student.name, email:student.email, contact:student.contact});
  };
  return (
    <div>
      <div>
...
<table className="table-fixed border border-green-600 hover:border-green-300 border-separate border-spacing-1 text-center">
  ...
  <tr>
    ...
    <th className="border border-blue-600 hover:border-blue-300 hover:text-white px-10 py-4">Contact</th>
  ...
  <tr key={a.id}>
    ...
    <td className="border border-blue-600 px-10 hover:border-blue-300 hover:text-white py-4">{a.contact}</td>
    ...
    <input type="text" name="email" id="email" value={data.email} onChange={(e) => setData({...data, email:e.target.value})} placeholder="Email"
    className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
    required/>
    </div>
    <div>
    <label htmlFor="contact" className="block mb-2 text-sm font-medium text-white dark:text-white">Contact</label>
    <input type="text" name="contact" id="contact" value={data.contact} onChange={(e) => setData({...data, contact:e.target.value})} placeholder="Contact"
    className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
    required/>
    </div>
